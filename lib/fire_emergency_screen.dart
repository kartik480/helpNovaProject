...
class _FireEmergencyScreenState extends State<FireEmergencyScreen> {
  ...
  Future<void> _submitRequest() async {
    ...
    if (_selectedFireType == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please select fire type')),
      );
      return;
    }

    if (_selectedSeverityLevel == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please select severity level')),
      );
      return;
    }
    ...
    try {
      ...
      // Call API to submit fire emergency request
      final result = await ApiService.submitFireEmergencyRequest(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Text('Your fire emergency report has been submitted successfully!'),
                  SizedBox(height: 10),
                  Text('Alerts sent to:'),
                  SizedBox(height: 5),
                  Text('• Nearby helpers'),
                  Text('• Fire services (future)'),
                ],
              ),
              actions: [
                TextButton(
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  hint: Text('Select fire type'),
                  items: _fireTypes.map((String type) {
                  ...
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return 'Please select fire type';
                    }
                    return null;
                  },
                ),
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: _severityLevels.map((String level) {
                    ...
                    if (level == 'High') {
                      ...
                      itemColor = Colors.yellow.shade700;
                    }
                    return DropdownMenuItem<String>(
                      value: level,
                      child: Row(
